...
const DOG_RUNTIME_TICK_SECONDS := 0.05
const DOG_RUNTIME_CYCLE_SECONDS := 9.5
...
var _dog_runtime_enabled := false
var _dog_runtime_time := 0.0
var _dog_runtime_dog: Dictionary = {}
...
func _read_user_args() -> void:
    for arg in OS.get_cmdline_user_args():
        ...
        elif arg == "--demo-dog-runtime":
            _dog_runtime_enabled = true
            _show_performance_hud = true
...
func _load_dog_runtime_data() -> void:
    if not _dog_runtime_enabled:
        return

    var text := FileAccess.get_file_as_string(DOG_DNA_PATH)
    if text.is_empty():
        push_warning("Could not read Dog DNA for companion dog runtime: %s" % DOG_DNA_PATH)
        _dog_runtime_dog = _fallback_dog_runtime_dog()
        return

    var parsed: Variant = JSON.parse_string(text)
    if typeof(parsed) != TYPE_DICTIONARY:
        push_warning("Could not parse Dog DNA for companion dog runtime: %s" % DOG_DNA_PATH)
        _dog_runtime_dog = _fallback_dog_runtime_dog()
        return

    var data := parsed as Dictionary
    var dogs := data.get("dogs", []) as Array
    for dog_value in dogs:
        var dog := dog_value as Dictionary
        if str(dog.get("dog_id", "")) == "DOG-PROT-001":
            _dog_runtime_dog = dog
            return

    push_warning("DOG-PROT-001 missing from Dog DNA; using companion dog runtime fallback.")
    _dog_runtime_dog = _fallback_dog_runtime_dog()


func _fallback_dog_runtime_dog() -> Dictionary:
    return {
        "dog_id": "DOG-PROT-001",
        "public_name": "Bublik",
        "morphology": {
...
func _start_dog_runtime() -> void:
    if not _dog_runtime_enabled or DisplayServer.get_name() == "headless":
        return

    var timer := Timer.new()
    timer.wait_time = DOG_RUNTIME_TICK_SECONDS
    timer.autostart = true
    timer.timeout.connect(_on_dog_runtime_tick)
    add_child(timer)
    _update_dog_runtime_label()


func _on_dog_runtime_tick() -> void:
    if _field_hidden:
        return

    _dog_runtime_time = fposmod(_dog_runtime_time + DOG_RUNTIME_TICK_SECONDS, DOG_RUNTIME_CYCLE_SECONDS)
    _update_dog_runtime_label()
    _apply_mouse_passthrough()
    queue_redraw()
...
func _build_overlay() -> void:
    ...
    _dog_runtime_label = Label.new()
    _dog_runtime_label.mouse_filter = Control.MOUSE_FILTER_IGNORE
    _dog_runtime_label.visible = _dog_runtime_enabled
    _dog_runtime_label.add_theme_color_override("font_color", Color(0.95, 0.98, 0.86, 1.0))
    _dog_runtime_label.add_theme_color_override("font_shadow_color", Color(0.0, 0.0, 0.0, 0.9))
    _dog_runtime_label.add_theme_constant_override("shadow_offset_x", 1)
    _dog_runtime_label.add_theme_constant_override("shadow_offset_y", 1)
    add_child(_dog_runtime_label)
...
func _apply_controls_scale() -> void:
    ...
    if _dog_runtime_label != null:
        var dog_label_size := Vector2(520.0, 48.0) * scale
        _dog_runtime_label.visible = _dog_runtime_enabled and not _field_hidden
        _dog_runtime_label.position = Vector2(132.0, 48.0) * scale
        _dog_runtime_label.custom_minimum_size = dog_label_size
        _dog_runtime_label.size = dog_label_size
        _apply_theme_scale(_dog_runtime_label, 10)
...
func _build_report() -> Array[String]:
    ...
    lines.append("dog_runtime_enabled=%s" % _dog_runtime_enabled)
    if _dog_runtime_enabled:
        var pose := _dog_runtime_pose()
        lines.append("dog_runtime_dog=%s/%s" % [
            str(_dog_runtime_dog.get("dog_id", "DOG-PROT-001")),
            str(_dog_runtime_dog.get("public_name", "Bublik")),
        ])
        lines.append("dog_runtime_phrase=%s" % str(pose.get("phrase", "idle")))
        lines.append("dog_runtime_socket=%s" % str(pose.get("socket", "mouth:empty")))
...
func _draw_dog_runtime() -> void:
    if not _dog_runtime_enabled:
        return

    var pose := _dog_runtime_pose()
    var root := _world_to_screen(Vector2(float(pose.get("x", 256.0)), 0.0))
    var scale := _dog_runtime_render_scale()
    var phase := float(pose.get("progress", 0.0))
    var moving := bool(pose.get("moving", false))
    var carrying := bool(pose.get("carrying", false))
    var wagging := str(pose.get("phrase", "")) == "wag"
    var body_bob := (sin(_dog_runtime_time * TAU * 2.1) * 2.0 * scale) if moving else (sin(_dog_runtime_time * TAU * 0.55) * 0.8 * scale)
    var tail_wag := sin(_dog_runtime_time * TAU * _dog_tail_wag_frequency()) * _dog_tail_wag_amplitude() * (1.7 if wagging else 1.0)
    var head_bob := sin((_dog_runtime_time * TAU * 1.25) + 0.7) * 1.6 * scale * _dog_head_bob_amount()
    var direction := 1.0
    var body_size := Vector2(_dog_body_length(), _dog_body_height()) * scale
    var leg_length := _dog_leg_length() * scale
    var body_center := root + Vector2(0.0, -leg_length - (body_size.y * 0.45) + body_bob)
    var base_color := _dog_base_color()
    var secondary_color := _dog_secondary_color()
    var outline_color := Color(0.14, 0.09, 0.06, 0.92)
    var shadow_rect := Rect2(root + Vector2(-body_size.x * 0.55, -4.0 * scale), Vector2(body_size.x * 1.08, 8.0 * scale))

    draw_rect(shadow_rect, Color(0.03, 0.02, 0.01, 0.28), true)

    var leg_roots := [
        body_center + Vector2(-body_size.x * 0.32, body_size.y * 0.28),
        body_center + Vector2(-body_size.x * 0.12, body_size.y * 0.32),
        body_center + Vector2(body_size.x * 0.18, body_size.y * 0.32),
        body_center + Vector2(body_size.x * 0.36, body_size.y * 0.28),
    ]

    for i in leg_roots.size():
        var offset := sin((_dog_runtime_time * TAU * _dog_step_frequency()) + (float(i) * PI * 0.75)) * 5.0 * scale if moving else 0.0
        var foot := Vector2(leg_roots[i].x + offset, root.y - (1.5 * scale))
        draw_line(leg_roots[i], foot, outline_color, maxf(2.0, 4.0 * scale))
        draw_line(leg_roots[i], foot, base_color.darkened(0.08), maxf(1.0, 2.4 * scale))
        draw_line(foot, foot + Vector2(direction * 8.0 * scale, 0.0), outline_color, maxf(1.0, 2.5 * scale))

    var body_rect := Rect2(body_center - (body_size * 0.5), body_size)
    draw_rect(body_rect, base_color, true)
    draw_circle(body_center + Vector2(-body_size.x * 0.5, 0.0), body_size.y * 0.5, base_color)
    draw_circle(body_center + Vector2(body_size.x * 0.5, 0.0), body_size.y * 0.5, base_color)
    draw_rect(body_rect, outline_color, false, maxf(1.0, 2.0 * scale))

    var chest_rect := Rect2(
        body_center + Vector2(body_size.x * 0.08, -body_size.y * 0.18),
        Vector2(body_size.x * 0.26, body_size.y * 0.52)
    )
    draw_rect(chest_rect, secondary_color, true)

    var neck := body_center + Vector2(body_size.x * 0.44 * direction, -body_size.y * 0.18)
    var head_radius := 17.0 * scale
    var head_center := neck + Vector2(19.0 * scale * direction, -10.0 * scale + head_bob)
    var muzzle_size := Vector2(22.0, 13.0) * scale
    var muzzle_rect := Rect2(
        head_center + Vector2((head_radius * 0.45) * direction, -muzzle_size.y * 0.42),
        muzzle_size
    )

    draw_line(body_center + Vector2(body_size.x * 0.34, -body_size.y * 0.18), neck, outline_color, maxf(5.0, 7.0 * scale))
    draw_circle(head_center, head_radius, base_color)
    draw_rect(muzzle_rect, secondary_color, true)
    draw_circle(head_center + Vector2(4.0 * scale, -4.0 * scale), 2.2 * scale, Color(0.05, 0.04, 0.035, 1.0))
    draw_circle(muzzle_rect.position + Vector2(muzzle_rect.size.x * 0.85, muzzle_rect.size.y * 0.42), 2.4 * scale, Color(0.04, 0.03, 0.03, 1.0))

    var ear_bounce := sin((_dog_runtime_time * TAU * 1.5) + phase) * 3.0 * scale * _dog_ear_bounce_amount()
    _draw_dog_ear(head_center + Vector2(-5.0 * scale, -head_radius * 0.78), scale, true, ear_bounce)
    _draw_dog_ear(head_center + Vector2(8.0 * scale, -head_radius * 0.72), scale, false, -ear_bounce * 0.5)

    var tail_base := body_center + Vector2(-body_size.x * 0.55, -body_size.y * 0.08)
    var tail_tip := tail_base + Vector2((-28.0 * scale), (-18.0 * scale) + (tail_wag * scale * 8.0))
    draw_line(tail_base, tail_tip, outline_color, maxf(3.0, 5.0 * scale))
    draw_line(tail_base, tail_tip, base_color.lightened(0.06), maxf(2.0, 3.0 * scale))

    _draw_dog_collar(neck, scale)

    var socket := _dog_mouth_socket(head_center, muzzle_rect, scale)
    if carrying:
        _draw_food_bag(socket + Vector2(9.0 * scale, 7.0 * scale), scale, true)
        draw_line(socket, socket + Vector2(9.0 * scale, 7.0 * scale), Color(0.14, 0.09, 0.05, 0.65), maxf(1.0, 1.5 * scale))
    elif str(pose.get("phrase", "")) in ["idle", "walk", "pickup"]:
        _draw_food_bag(_world_to_screen(Vector2(_dog_pickup_world_x(), 0.0)) + Vector2(0.0, -9.0 * scale), scale, false)
    else:
        _draw_food_bag(_world_to_screen(Vector2(_dog_delivery_world_x(), 0.0)) + Vector2(0.0, -9.0 * scale), scale, false)


func _draw_dog_ear(anchor: Vector2, scale: float, upright: bool, bounce: float) -> void:
    var color := _dog_base_color().darkened(0.08)
    if upright:
        var points := PackedVector2Array([
            anchor + Vector2(-5.0, 3.0) * scale,
            anchor + Vector2(2.0, -25.0) * scale + Vector2(0.0, bounce),
            anchor + Vector2(10.0, 3.0) * scale,
        ])
        draw_colored_polygon(points, color)
        draw_polyline(points, Color(0.12, 0.08, 0.05, 0.95), maxf(1.0, 1.4 * scale), true)
        return

    var points := PackedVector2Array([
        anchor + Vector2(-5.0, 0.0) * scale,
        anchor + Vector2(7.0, 8.0) * scale + Vector2(0.0, bounce),
        anchor + Vector2(8.0, 28.0) * scale + Vector2(0.0, bounce),
        anchor + Vector2(-4.0, 18.0) * scale,
    ])
    draw_colored_polygon(points, color)
    draw_polyline(points, Color(0.12, 0.08, 0.05, 0.95), maxf(1.0, 1.4 * scale), true)


func _draw_dog_collar(neck: Vector2, scale: float) -> void:
    var collar_color := Color(0.33, 0.61, 0.45, 1.0)
    var rect := Rect2(neck + Vector2(-2.0, -13.0) * scale, Vector2(8.0, 24.0) * scale)
    draw_rect(rect, collar_color, true)
    draw_rect(rect, Color(0.08, 0.12, 0.08, 0.8), false, maxf(1.0, 1.2 * scale))


func _draw_food_bag(center: Vector2, scale: float, attached: bool) -> void:
    var bag_size := Vector2(18.0, 22.0) * scale
    var rect := Rect2(center - (bag_size * 0.5), bag_size)
    var fill := Color(0.73, 0.54, 0.30, 1.0) if attached else Color(0.65, 0.45, 0.24, 0.95)
    var outline := Color(0.18, 0.11, 0.05, 0.95)
    var points := PackedVector2Array([
        rect.position + Vector2(rect.size.x * 0.18, rect.size.y * 0.10),
        rect.position + Vector2(rect.size.x * 0.82, rect.size.y * 0.10),
        rect.position + Vector2(rect.size.x * 0.94, rect.size.y * 0.88),
        rect.position + Vector2(rect.size.x * 0.08, rect.size.y * 0.88),
    ])
    draw_colored_polygon(points, fill)
    draw_polyline(points, outline, maxf(1.0, 1.6 * scale), true)
    draw_line(rect.position + Vector2(rect.size.x * 0.25, rect.size.y * 0.25), rect.position + Vector2(rect.size.x * 0.75, rect.size.y * 0.25), outline, maxf(1.0, 1.2 * scale))
...
func _placeholder_screen_rects() -> Array[Rect2]:
    ...
    for rect in _dog_runtime_screen_rects():
        rects.append(rect)
...
func _dog_runtime_pose() -> Dictionary:
    var t := fposmod(_dog_runtime_time, DOG_RUNTIME_CYCLE_SECONDS)
    var start_x := _dog_start_world_x()
    var pickup_x := _dog_pickup_world_x()
    var delivery_x := _dog_delivery_world_x()

    if t < 1.2:
        return _dog_pose("idle", "idle_base", "mouth:empty", start_x, t / 1.2, false, false)

    t -= 1.2
    if t < 2.1:
        return _dog_pose("walk", "walk_base + procedural step/head/ear/tail", "mouth:empty", lerpf(start_x, pickup_x, _smooth_step(t / 2.1)), t / 2.1, true, false)

    t -= 2.1
    if t < 0.9:
        return _dog_pose("pickup", "pickup_base + socket align", "mouth:food_bag_attach", pickup_x, t / 0.9, false, t > 0.45)

    t -= 0.9
    if t < 2.3:
        return _dog_pose("carry", "carry_walk_base + procedural effort", "mouth:food_bag_attached", lerpf(pickup_x, delivery_x, _smooth_step(t / 2.3)), t / 2.3, true, true)

    t -= 2.3
    if t < 0.9:
        return _dog_pose("deliver", "deliver_base + socket release", "mouth:food_bag_release", delivery_x, t / 0.9, false, t < 0.45)

    t -= 0.9
    if t < 1.3:
        return _dog_pose("wag", "wag_base + procedural tail/head/ear", "mouth:empty", delivery_x, t / 1.3, false, false)

    return _dog_pose("idle", "idle_base", "mouth:empty", start_x, (t - 1.3) / 0.8, false, false)


func _dog_pose(phrase: String, clip: String, socket: String, x: float, progress: float, moving: bool, carrying: bool) -> Dictionary:
    return {
        "phrase": phrase,
        "clip": clip,
        "socket": socket,
...
func _dog_runtime_screen_rects() -> Array[Rect2]:
    var rects: Array[Rect2] = []
    if not _dog_runtime_enabled:
        return rects

    var pose := _dog_runtime_pose()
    var root := _world_to_screen(Vector2(float(pose.get("x", _dog_start_world_x())), 0.0))
    var scale := _dog_runtime_render_scale()
    rects.append(Rect2(root + Vector2(-92.0, -108.0) * scale, Vector2(184.0, 116.0) * scale))

    var bag_x := _dog_pickup_world_x() if str(pose.get("phrase", "")) in ["idle", "walk", "pickup"] else _dog_delivery_world_x()
    rects.append(Rect2(_world_to_screen(Vector2(bag_x, 0.0)) + Vector2(-18.0, -32.0) * scale, Vector2(36.0, 36.0) * scale))
    return rects


func _update_dog_runtime_label() -> void:
    if _dog_runtime_label == null or not _dog_runtime_enabled:
        return

    var pose := _dog_runtime_pose()
    var dog_id := str(_dog_runtime_dog.get("dog_id", "DOG-PROT-001"))
    var public_name := str(_dog_runtime_dog.get("public_name", "Bublik"))
    _dog_runtime_label.text = "%s / %s | phrase %s | socket %s\nclip %s | bridge Dog DNA -> embedded companion runtime" % [
        dog_id,
        public_name,
        str(pose.get("phrase", "idle")),
        str(pose.get("socket", "mouth:empty")),
        str(pose.get("clip", "idle_base")),
    ]
...
func _dog_start_world_x() -> float:
    return 8.0 * _cell_size


func _dog_pickup_world_x() -> float:
    return 16.0 * _cell_size


func _dog_delivery_world_x() -> float:
    return 24.0 * _cell_size


func _dog_runtime_render_scale() -> float:
    return _zoom()


func _dog_morphology() -> Dictionary:
    return _dog_runtime_dog.get("morphology", {}) as Dictionary


func _dog_appearance() -> Dictionary:
    return _dog_runtime_dog.get("appearance", {}) as Dictionary


func _dog_animation_profile() -> Dictionary:
    return _dog_runtime_dog.get("animation_profile", {}) as Dictionary


func _dog_body_length() -> float:
    match str(_dog_morphology().get("body_length", "medium")):
        "long":
            return 98.0
        "short":
            return 68.0
...
func _dog_body_height() -> float:
    match str(_dog_morphology().get("body_volume", "medium")):
        "sturdy":
            return 38.0
        "light":
            return 28.0
...
func _dog_leg_length() -> float:
    match str(_dog_morphology().get("leg_length", "medium")):
        "short":
            return 19.0
        "medium_long":
            return 32.0
...
func _dog_step_frequency() -> float:
    return float(_dog_animation_profile().get("step_frequency", 1.1))


func _dog_head_bob_amount() -> float:
    return float(_dog_animation_profile().get("head_bob_amount", 0.8))


func _dog_tail_wag_amplitude() -> float:
    return float(_dog_animation_profile().get("tail_wag_amplitude", 1.2))


func _dog_tail_wag_frequency() -> float:
    return float(_dog_animation_profile().get("tail_wag_frequency", 1.1))


func _dog_ear_bounce_amount() -> float:
    return float(_dog_animation_profile().get("ear_bounce_amount", 1.0))


func _dog_base_color() -> Color:
    return _dog_color_from_name(str(_dog_appearance().get("base_coat", "warm_brown")), Color(0.56, 0.31, 0.15, 1.0))


func _dog_secondary_color() -> Color:
    return _dog_color_from_name(str(_dog_appearance().get("secondary_coat", "cream")), Color(0.88, 0.74, 0.53, 1.0))


func _dog_color_from_name(color_name: String, fallback: Color) -> Color:
    match color_name:
        "warm_brown":
            return Color(0.56, 0.32, 0.16, 1.0)
        "dark_brown":
            return Color(0.25, 0.14, 0.08, 1.0)
        "cream":
            return Color(0.89, 0.76, 0.53, 1.0)
        "white_warm":
            return Color(0.91, 0.86, 0.75, 1.0)
        "golden":
            return Color(0.72, 0.49, 0.18, 1.0)
        _:
            return fallback


func _dog_mouth_socket(head_center: Vector2, muzzle_rect: Rect2, scale: float) -> Vector2:
    return Vector2(muzzle_rect.end.x, head_center.y + (4.0 * scale))
