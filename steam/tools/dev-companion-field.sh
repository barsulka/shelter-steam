...
case "$MODE" in
    ...
    perf)
        ...
        ;;
    dog-runtime)
        exec "$GODOT_BIN" --print-fps --path "$ROOT_DIR" --scene res://scenes/tech_demos/companion_field_demo.tscn -- --demo-companion --demo-transparent --demo-dog-runtime --demo-perf
        ;;
    dog-runtime-smoke)
        exec "$GODOT_BIN" --headless --path "$ROOT_DIR" --scene res://scenes/tech_demos/companion_field_demo.tscn -- --demo-auto-quit --demo-seconds=1.2 --demo-companion --demo-dog-runtime --demo-perf
        ;;
    *)
        echo "Usage: tools/dev-companion-field.sh [all|interactive|companion|normal|click-through|perf|dog-runtime|dog-runtime-smoke|smoke]" >&2
        exit 2
        ;;
esac
