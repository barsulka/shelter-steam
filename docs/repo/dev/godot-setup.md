...
For normal manual launches, use the visual launcher:

```sh
./launch.sh
```

The `tools/*.sh` scripts remain development diagnostics and smoke-check helpers.

## Manual Checks

```sh
"$GODOT_BIN" --version
"$GODOT_BIN" --headless --path steam --import
"$GODOT_BIN" --headless --path steam --quit-after 2
"$GODOT_BIN" --headless --path steam --check-only --script res://scripts/main.gd
```
