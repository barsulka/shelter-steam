...
- 2026-07-02: для Steam/Desktop dev и ChatGPT inspection workflows появился sibling repo Shelter MCP: `/Users/barsulka/GolandProjects/shelter/mcp` (`git@github.com:barsulka/shelter-mcp.git`). Это preferred bridge для remote/local AI tool access: один MCP endpoint вместо старой схемы “отдельный filesystem MCP tunnel + отдельные игровые запускалки”. Он exposes whitelisted Shelter tools для Workbench Runtime Capture, capture-run management, local Godot State Connector control runtime, runtime control actions и `fs_*` proxy к official filesystem MCP.
