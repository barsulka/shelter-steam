...
tools/run-companion-field-demo.sh companion
...
- The demo uses curated placeholder sprites from `assets/tech_demos/placeholder_art/`: `TX Tileset Ground`, `TX Village Props`, and all outlined `MinifolksForestAnimals` species. Current field objects use configured `TX Village Props` atlas regions instead of procedural tower/fountain/bin shapes. The animals move on short low-frequency timer paths and animate from explicit tight non-empty frame lists so the strip feels alive and the sprites stay readable without adding a permanent `_process()` loop. The local raw `tilesets/` source drop is ignored by Git and has a tracked `.gdignore` so Godot does not import large source packs.
...
The current demo uses a small curated placeholder-art subset from `assets/tech_demos/placeholder_art/` instead of committing the whole raw `tilesets/` source directory. The raw source directory is intentionally ignored by Git because it can contain large packs, editor files, and alternate styles that the demo does not need. Keep `tilesets/.gdignore` tracked so Godot skips local raw packs during import/check commands.
...
CQ Hero Town reference notes now live in `.agents/skills/cq-hero-town-reference/`.
...
- At least one multi-monitor placement test is recorded in `docs/status/CODEX_STATUS.md`.
