...
- Shelter MCP не является обязательным файловым bridge. Он используется только как локальный domain-specific runtime / inspection adapter для Workbench, Godot connector/control и ограниченной навигации по знаниям.
...
Shelter MCP Knowledge tools можно использовать как необязательный bounded navigator, если они подключены и экономят чтение:
...
MCP digest/catalog не заменяет source docs. При расхождении всегда открывай локальный документ и считай его источником истины.
