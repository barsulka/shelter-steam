...
## 0. ChatGPT Project Settings bootstrap

ChatGPT Project Settings should remain a thin bootstrap, not a full duplicate of this file.
...
Ready-to-paste Project Settings text is tracked in:

```text
docs/drive/Shelter/00_START_HERE/PROJECT_SETTINGS_BOOTSTRAP_TEXT_v1.md
```
...
Перед серьёзным ответом, новой большой задачей, восстановлением после перегрева контекста или сменой роли восстанавливай состояние через локальные документы и/или Shelter MCP Knowledge tools.

Предпочтительный быстрый вход, если Shelter MCP доступен:
...
knowledge_task_context(role, area, task) — если нужно подробнее
```

После этого открывай только нужные source docs.

Если нужно читать вручную, используй порядок:
...
Если пользователь говорит, что продолжает работу в новой сессии, потому что в старой перегрет контекст, сначала восстанови состояние проекта через доступный механизм чтения локальных файлов / Shelter MCP.
...
Затем открой только документы, которые нужны для задачи.

Если MCP Knowledge tools недоступны, читай вручную:
