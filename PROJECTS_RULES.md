...
- ChatGPT-сессии работают с локальными документами через подключённый инструмент `локальный файлсервер`. Если `локальный файлсервер` недоступен, ChatGPT обязан остановиться и попросить пользователя подключить его.
...
## 9. Порционный доступ к локальный файлсервер

В ChatGPT-сессиях `локальный файлсервер` может иметь технический лимит или временную блокировку на чтение/запись файлов.
...
5. попросить пользователя снова включить `локальный файлсервер`, формулировкой вроде: “вруби плагин”;
